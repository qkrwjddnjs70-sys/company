...
2. **급상승 키워드 발견** — `config/trendbot.json`에 등록해 둔 후보 키워드(카테고리별
   ...
   보여준다. 기본은 증가율 문턱값(기본 +30%)을 넘는 것만 보여주므로 대부분 몇 건 안
   나올 수 있다 — 웹 UI의 **"전체 후보를 카테고리별로 다 보기"** 체크박스를 켜면
   문턱값과 상관없이 데이터가 있는 후보 전부를 카테고리별로 묶어서 보여준다
   (CLI는 `spikes --all`).
...
- **검색광고 API의 "월간검색수" 집계 기간 ⚠️.** "이번 달 1일~말일"처럼 달력 월 단위가
  아니라, 조회 시점 기준으로 매일 굴러가는 최근 30일의 실제 검색 횟수로 알려져 있다
  (평균 아님). 다만 정확히 "몇 시 기준, 며칠 전까지"인지는 네이버가 공식 문서로
  못 박아두지 않아, 이는 실무에서 통용되는 이해이지 공식 확인 사항은 아니다. 그래서
  오늘과 내일 조회한 값이 하루이틀 차이로 미세하게 달라질 수 있다 — 정상이다.
